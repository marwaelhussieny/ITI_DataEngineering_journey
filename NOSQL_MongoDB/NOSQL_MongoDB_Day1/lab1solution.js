...
// 2. Create "Staff" collection and insert documents
// Insert one document
db.Staff.insertOne({
    _id: 1,
    name: "Marwa",
...
db.Staff.insertMany([
    {_id: 2, name: "Ahmed", age: 20, gender: "male", department: "Finance"},
    {_id: 3, name: "Sara", age: 25, gender: "female", managerName: "Ali", department: "HR"},
    {_id: 4, name: "Omar", age: 15, gender: "male", DOB: "2008-01-01"},
    {_id: 5, name: "Laila", age: 15, gender: "female", DOB: "2008-05-10"}
]);
...
// 3.2 Find documents where gender is "male"
db.Staff.find({gender: "male"})

// 3.3 Find documents with age between 20 and 25
db.Staff.find({age: {$gte: 20, $lte: 25}})

// 3.4 Find documents where age is 25 and gender is "female"
db.Staff.find({age: 25, gender: "female"})

// 3.5 Find documents where age is 20 OR gender is "female"
db.Staff.find({$or: [{age: 20}, {gender: "female"}]})

// 4. Update one document where age is 15, set the name to "Marwa"
db.Staff.updateOne({age: 15}, {$set: {name: "Marwa"}})

// Verify the update
db.Staff.find({ age: 15 })

// 5. Update many documents, set department to "AI"
db.Staff.updateMany({}, {$set: {department: "AI"}})

// Verify the update
db.Staff.find({}, { name: 1, department: 1 })

// 6. Create a new collection "test" and insert same documents as before
db.test.insertMany([
    {_id: 2, name: "Ahmed", age: 20, gender: "male", department: "Finance"},
    {_id: 3, name: "Sara", age: 25, gender: "female", managerName: "Ali", department: "HR"},
    {_id: 4, name: "Omar", age: 15, gender: "male", DOB: "2008-01-01"},
...
// 7. Delete one document from "test" where age = 15
db.test.deleteOne({age: 15})

// Verify deletion
db.test.find({ age: 15 })  // Should return empty

// Check remaining documents
db.test.find()

// 7a. Justification:
// If more than one document has age = 15, deleteOne() will delete the **first document MongoDB finds** that matches the query. 
// It is not guaranteed which one will be deleted unless you add a sort or specify _id.